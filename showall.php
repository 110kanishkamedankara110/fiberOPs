...
INNER JOIN `condition` ON `condition`.`id`=`product`.`condition_id`  WHERE `category_id`='".$cat."' AND `status_id`!='2' LIMIT " . $limit . " OFFSET " . $ofs . " ;");
...
INNER JOIN `brand` ON `brand`.`id`=`product`.`brand_id` 
INNER JOIN `color` ON `color`.`id`=`product`.`color_id` 
INNER JOIN `condition` ON `condition`.`id`=`product`.`condition_id`  WHERE `category_id`='".$cat."' AND `status_id`!='2' ;");
